...
import { getDomainsWithOutHits, deleteDomains } from "@/lib/domain-helper";
...
const WithoutHits = ({ tData,deleteDomain }: tableProps) => {
  ...
  const deleteRows = async () => {
    ...
    selected.map((row) => ids.push(row.id));
  ...
  useEffect(() => {
    ...
    const getAllDomains = async () => {
      ...
      const res = await getDomainsWithOutHits(
        search,
        limit,
        page,
        sortBy,
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <div className="flex items-center space-x-3.5">
                          <Link
                            ...
                            replace
                            className="w-8 h-8 inline-flex items-center justify-center rounded border border-[#eee] hover:bg-strokedark hover:text-white hover:border-strokedark"
                          >
                            <svg
