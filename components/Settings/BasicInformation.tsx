...
export default function BasicInformation(userdetails: any)  {
  const { data: session } = useSession();
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');
  const [updating, setUpdating] = useState(false);
  const userFirstLast = userdetails as details;
  const userInfo = userFirstLast.userdetails ;

  const schema = Yup.object().shape({
    firstName: Yup.string().required('Enter your email address.'),
    lastName: Yup.string().required('Enter your email address.'),
  });
  ...
  const handleClick = async (values: User) => {
    ...
    method: 'POST',
    body: JSON.stringify({ first_name:values.firstName, last_name:values.lastName, token:session?.token  }),
    });
