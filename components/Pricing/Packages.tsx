...
const Packages = (props: PackagesProps) => {
  return (
    <div
      key={props.id}
      className="group relative bg-white dark:bg-black rounded-lg shadow-solid-10 dark:shadow-none border border-stroke dark:border-black p-7.5 xl:p-12.5"
    >
        ...
        { props.id !== 1 && (
          <>
            <a 
              ...
              className={props.id === props.userPlanId ? 'hover:pr-2 duration-500 disabled' : 'hover:pr-2 duration-500'}
              onClick={(e) => props.id === props.userPlanId && e.preventDefault()}
            >
              <button type="button" className="hover:pr-2 duration-500">
                {
